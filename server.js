...
const openaiApiKey = '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';
...
passport.use(new GoogleStrategy({
  clientID: '1094942275851-hrs9kv1q317dhrkekigab0k3qrui9vif.apps.googleusercontent.com',
  clientSecret: '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',
...
app.get('/auth/google/callback',
  ...
  (req, res) => {
    // Clear the session before redirecting the user
    res.redirect('/inbox');
  });

// Route to fetch recent inbox emails
...
app.get('/inbox', async (req, res) => {
  ...
  // Retrieve the user's inbox data from the database
  ...
  try {
    ...
    // Fetch the user's most recent email
    const response = await gmail.users.messages.list({
      userId: 'me',
      maxResults: 1,
      q: 'in:inbox', // You can use other search parameters to narrow down the search
    });
    ...
    // Here, you can store the emailData in MongoDB or process it as needed
  ...
  try {
    ...
    for (const user of users) {
      ...
      const response = await gmail.users.messages.list({
        userId: 'me',
        maxResults: 1,
        q: 'is:unread in:inbox category:primary after:2023/07/20 -from:noreply*', // Modify the search query to fetch unread emails in the inbox
      });
      ...
      if (response.data.messages && response.data.messages.length > 0) {
        ...
        // ... The rest of your existing code to process the email and send the reply
        // Use the same logic as in the /inbox route

        // Example of sending the reply:
        // const recipient = emailData.senderEmail;
        // await sendEmailReply(emailData, recipient, modelOutput, oauth2Client);
